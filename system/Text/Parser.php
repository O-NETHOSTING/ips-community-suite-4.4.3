...
class _Parser
{
	...
	{		
		...
		foreach( $contentImages as $element )
		{
			if ( $element->hasAttribute('src') )
			{
				...
				{
					$element->setAttribute( 'data-ratio', (float) round( ( $attachment['attach_img_height'] / $attachment['attach_img_width'] ) * 100, 2 ) );
				}
			}
		}
	...
	{		
		...
		foreach( $contentVideos as $element )
		{
			if ( $element->hasAttribute('data-video-src') )
			{
	...
	protected function _parseElement( \DOMElement $element )
	{
		...
		switch ( $element->tagName )
		{
			...
			case 'source':
				$element = $this->_parseSourceElement( $element );
				break;
		}
	 ...
	 * @return	\DOMElement
	 */
	protected function _parseSourceElement( \DOMElement $element )
	{
	...
	protected static function _oembedEmbed( \IPS\Http\Url $url, $iframe=FALSE )
	{
		...
		if( $entry )
		{
			...
			/* Flickr does not set target='_blank' on its links, which results in them loading inside the iframe but failing due to X-Frame-Options */
