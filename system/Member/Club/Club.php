...
class _Club extends \IPS\Patterns\ActiveRecord implements \IPS\Content\Embeddable
{
	...
	public function tabs( \IPS\Node\Model $container = NULL )
	{
		...
		{
			$tabs = array();
			$tabs[ 'club_home' ] = array( 'href' => $this->url()->setQueryString('do', 'overview'), 'title' => \IPS\Member::loggedIn()->language()->addToStack( 'club_home' ), 'isActive' => ( \IPS\Request::i()->module == 'clubs' AND ( !\IPS\Request::i()->do OR \IPS\Request::i()->do == 'overview' ) ) ? TRUE : FALSE  );
