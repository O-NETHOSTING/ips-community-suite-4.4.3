...
{	
	...
	/**
	 * @brief	If we are viewing a member's profile, which member
	 */
	protected $memberProfile = NULL;
	 ...
	 * @return	void
	 */
	public function init( $member=NULL )
	{
		/* Init */
		...
		foreach ( $this->getArrayCopy() as $result )
		{
			...
			if ( $result['index_container_id'] )
			{
				...
				else
				{
					...
					if ( $result['index_container_class'] )
					{
						$containerClass = $result['index_container_class'];
						if ( isset( $containerClass::$seoTitleColumn ) )
						{
							$containersToLoad[ $result['index_container_class'] ][ $result['index_container_id'] ] = $result['index_container_id'];
						}
					}
				}
			}
		...
		foreach ( $itemsToLoad as $itemClass => $itemIds )
		{
			...
			{
				foreach ( $itemClass::searchResultExtraData( $this->itemData[ $itemClass ], $this->memberProfile ) as $id => $extraData )
				{
	...
	public function current()
	{
		...
		$itemClass = \in_array( 'IPS\Content\Comment', class_parents( $class ) ) ? $class::$itemClass : $class;
		$containerClass = $data['index_container_class'] ?: NULL;
