...
{	
	...
	public function filterByContent( array $contentFilters, $type = TRUE )
	{
		...
		foreach ( $contentFilters as $filter )
		{
			...
			if( $filter->containerClasses !== NULL )
			{
				...
				{
					if( $filter->containerOwnedExclusions !== NULL )
					{
						$clause[] = '(' . \IPS\Db::i()->in( 'index_container_class', $filter->containerClasses ) . ' OR ' . \IPS\Db::i()->in( 'index_class', $filter->containerClassExclusions ) . ' OR (' . \IPS\Db::i()->in( 'index_container_class', $filter->containerOwnedExclusions[0] ) . ' AND ' . \IPS\Db::i()->in( 'index_container_id', $filter->containerOwnedExclusions[1], TRUE ) . '))';
					}
					else
					{
	...
	public function filterForProfile( \IPS\Member $member )
	{
		...
		foreach( $classesChecked as $itemClass )
		{
			$itemClass::searchEngineFiltering( $filters, $filterResult, $member );
		}
	 ...
	 * @param	array|NULL	$containerExclusions	Return results if the content is not within these containers
	 * @return	\IPS\Content\Search\Query	(for daisy chaining)
	 */
	public function filterByContainerClasses( $classes=array(), $exclude=array(), $containerExclusions=NULL )
	{
		if( empty( $exclude ) )
		{
			if( \is_array( $containerExclusions ) AND \count( $containerExclusions ) )
			{
				$this->where[] = '( index_container_class IS NULL OR ' . \IPS\Db::i()->in( 'index_container_class', $classes, TRUE ) . ') OR (' . \IPS\Db::i()->in( 'index_container_class', $classes ) . ' AND ' . \IPS\Db::i()->in( 'index_container_id', $containerExclusions, TRUE ) . ')';
			}
			else
			{
		...
		else
		{
			...
			if( \is_array( $containerExclusions ) AND \count( $containerExclusions ) )
			{
				$this->where[] = '( index_container_class IS NULL OR index_container_class NOT IN(' . implode( ',', $classes ) . ') OR index_class IN(' . implode( ',', $exclude ) . ') OR (index_container_class IN(' . implode( ',', $classes ) . ') AND ' . \IPS\Db::i()->in( 'index_container_id', $containerExclusions, TRUE ) . '))';
			}
			else
			{
