...
abstract class _Item extends \IPS\Content
{
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		if ( !\IPS\Member::loggedIn()->member_id )
		{
			...
			{
				$return['guest_email'] = new \IPS\Helpers\Form\Email( 'guest_email', NULL, TRUE, array( 'accountEmail' => TRUE ), NULL, NULL, NULL, 'guest_email' );
			}
			if ( !$container or $guestsCanPostInContainer )
			{
				if ( isset( static::$databaseColumnMap['author_name'] ) )
				{
					$return['guest_name']	= new \IPS\Helpers\Form\Text( 'guest_name', NULL, FALSE, array( 'minLength' => \IPS\Settings::i()->min_user_name_length, 'maxLength' => \IPS\Settings::i()->max_user_name_length, 'placeholder' => \IPS\Member::loggedIn()->language()->addToStack('comment_guest_name') ), NULL, NULL, NULL, 'guest_name' );
				}
			}
	...
	public function commentForm( $lastSeenId = NULL )
	{
		...
		if ( $this->canComment() )
		{
			...
			/* Mod Queue? */
			$return = '';
			$guestPostBeforeRegister = ( !\IPS\Member::loggedIn()->member_id ) ? ( !isset( static::$containerNodeClass ) or ( $container = $this->container() and !$container->can( 'add', \IPS\Member::loggedIn(), FALSE ) ) ) : NULL;
	...
	public function commentFormElements()
	{
		...
		if ( !\IPS\Member::loggedIn()->member_id )
		{
			...
			{
				$return['guest_email'] = new \IPS\Helpers\Form\Email( 'guest_email', NULL, TRUE, array( 'accountEmail' => TRUE, 'placeholder' => \IPS\Member::loggedIn()->language()->addToStack('comment_guest_email') ) );
			}
			else
			{
	...
	public function reviewForm()
	{
		...
		if ( $this->canReview() )
		{
			...
			if ( !\IPS\Member::loggedIn()->member_id )
			{
				...
				{
					$form->add( new \IPS\Helpers\Form\Email( 'guest_email', NULL, TRUE, array( 'accountEmail' => TRUE ) ) );
				}
				else
				{
	...
	public function hasReviewed( $member=NULL )
	{
		...
		if( !isset( $this->_hasReviewed[ $member->member_id ] )  )
		{
			...
			if ( $member === NULL )
			{
				...
				{
					$this->_hasReviewed[ $member->member_id ] = \IPS\Db::i()->select( 'COUNT(*)', 'core_post_before_registering', array( 'class=? AND id=? AND secret=?', $reviewClass, $this->$idColumn,  \IPS\Request::i()->cookie['post_before_register'] ) )->first();
				}
				else
				{
