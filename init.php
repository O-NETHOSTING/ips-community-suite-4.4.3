...
{	
	...
	public static function defaultConstants()
	{
		...
		if ( isset( self::$cicConfig['remote-cache'] ) and self::$cicConfig['remote-cache']['mode'] == 'on' )
		{
			...
			$outputCacheConfig = json_encode( array(
				'sql_host'     => self::$cicConfig['remote-cache']['endpoint'],
				'sql_database' => $INFO['sql_database'],
				'sql_user'     => $INFO['sql_user'],
				'sql_pass'     => $INFO['sql_pass'],
				'sql_port' 	   => ( ! empty( $INFO['sql_port'] ) )   ? $INFO['sql_port'] : NULL,
				'sql_socket'   => ( ! empty( $INFO['sql_socket'] ) ) ? $INFO['sql_socket'] : NULL,
				'sql_tbl_prefix' => ( ! empty( $INFO['sql_tbl_prefix'] ) ) ? $INFO['sql_tbl_prefix'] : ''
			) );
		...
		if ( isset( self::$cicConfig['redis'] ) and self::$cicConfig['redis']['mode'] !== 'off' )
		{
			...
			if ( isset( self::$cicConfig['redis']['replicas'] ) and \count( self::$cicConfig['redis']['replicas'] ) and self::$cicConfig['redis']['mode'] == 'replica' )
			{
				/* RW separation baby */
				$redisConfig = array(
					'write' => array(
						'server' => self::$cicConfig['redis']['primary'],
						'port'   => 6379
					),
					'read' => array()
				);
				
				foreach( self::$cicConfig['redis']['replicas'] as $replica )
				{
	 ...
	 * @return	void
	 */
	public static function applyLatestFilesIPSCloud()
	{
		if ( \IPS\CIC )
		{
			if ( preg_match( '/^\/var\/www\/html\/(.+?)(?:\/|$)/i', \IPS\ROOT_PATH, $matches ) )
			{
				try
				{
					\IPS\Http\Url::external('http://ips-cic-fileupdate.invisioncic.com/applylatestfiles.php')
						->setQueryString( array( 'user' => $matches[1], 'key' => md5( $matches[1] . \IPS\Settings::i()->sql_pass ) ) )
						->request()
						->post();
				}
				catch ( \Exception $e ) { }
