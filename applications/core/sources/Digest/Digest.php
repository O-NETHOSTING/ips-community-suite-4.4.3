...
class _Digest
{
	 ...
	 * @return	bool
	 */
	public static function sendDigestBatch( $frequency='daily', $numberToSend=NULL )
	{
		/* Maximum to send */
		if( $numberToSend === NULL )
		{
			$classToUse = \IPS\Email::classToUse( \IPS\Email::TYPE_LIST );
			$numberToSend = $classToUse::MAX_EMAILS_PER_GO;
		}
		...
		$follows = \IPS\Db::i()->select( '*', 'core_follow', array( 'follow_notify_do=1 AND follow_notify_freq=? AND follow_notify_sent < ? AND ' . \IPS\Db::i()->in( 'follow_member_id', $memberIDs ), $frequency, ( $frequency == 'daily' ) ? time() - 86400 : time() - 604800 ), 'follow_notify_sent ASC, follow_app ASC, follow_area ASC' );
