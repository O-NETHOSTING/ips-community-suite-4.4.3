...
class _cleanup extends \IPS\Task
{
	...
	public function execute()
	{
		...
		if ( \IPS\Settings::i()->subs_autoprune )
		{
			...
			{
				if ( \in_array( 'IPS\Content\Followable', class_implements( $class ) ) AND isset( $class::$databaseColumnMap['last_comment'] ) )
				{
					...
					$select = \IPS\Db::i()->select( 'follow_id', 'core_follow', array( 'core_follow.follow_app=? AND core_follow.follow_area=? AND ' . $class::$databaseTable . '.' . $lastPostColumn . '<?', $application, $area, \IPS\DateTime::create()->sub( new \DateInterval( 'P' . \IPS\Settings::i()->subs_autoprune . 'D' ) )->getTimestamp() ) )
