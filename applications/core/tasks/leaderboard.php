...
class _leaderboard extends \IPS\Task
{
	...
	public function execute()
	{
		...
		for( $i = 0; $i < $difference; $i++ )
		{
			...
			foreach( \IPS\Db::i()->select( 'core_reputation_index.member_received as member, SUM(rep_rating) as rep', 'core_reputation_index', $where, 'rep DESC, member ASC', 4, 'member' )->join( 'core_members', 'core_members.member_id=core_reputation_index.member_received' )->setKeyField('member')->setValueField('rep') as $member => $rep )
			{
				if ( $member and $rep )
				{
					$date = $startDate->setTime( 12, 0 )->getTimeStamp();
