...
abstract class _Library
{
	...
	public function process( $data, $method, $perCycle=NULL )
	{
		...
		if ( $data >= $total )
		{
			...
			$percentage	= 100 / array_sum( $_SESSION['convertCountRows'] ) * $this->_getConvertedCount();
		...
		else
		{
			...
			if( $this->software->app->getRunningFlag( $method ) )
			{
				...
				sleep(5);
				return array( $data, \IPS\Member::loggedIn()->language()->get( 'waiting_previous_process' ) . $dots, 100 / array_sum( $_SESSION['convertCountRows'] ) * $this->_getConvertedCount( $total ) );
			}
			...
			$percentage	= 100 / array_sum( $_SESSION['convertCountRows'] ) * $this->_getConvertedCount( ( $data + static::$perCycle > $total ) ? $total : $data + static::$perCycle );
	...
	protected function _getConvertedCount( $add = 0 )
	{
		...
		foreach( $applicationsToCheck as $app )
		{
			$totalSoFar += array_sum(
				array_filter( $_SESSION['convertCountRows'], function( $key ) use( $app ) {
