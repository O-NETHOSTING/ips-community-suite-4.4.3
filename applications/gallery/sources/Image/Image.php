...
\IPS\Content\MetaData
{
	...
	{
		return $this->album_id ?: $this->category_id;
	}
	 ...
	 * @param	\IPS\Member|NULL			$member		The member we are filtering by (if showing on profile)
	 * @return	void
	 */
	public static function searchEngineFiltering( &$filters, &$query, $member=NULL )
	{
		...
		{
			/* If we are showing content owned by someone (i.e. for profile), then we need to show images in albums we don't own */
			$albums = NULL;

			if( $member !== NULL )
			{
				$albums		= iterator_to_array( \IPS\Db::i()->select( 'album_id', 'gallery_albums', array( 'album_owner_id=?', $member->member_id ) ) );
			}
			...
			if( \count( $filters ) )
			{
				foreach( $filters as $k => $filter )
				{
					if( $filter->itemClass == 'IPS\\gallery\\Image' )
					{
						...
						$filter->containerOwnedExclusions = array( array( 'IPS\\gallery\\Album' ), $albums );
					}
				}
			}
			else
			{
				$query->filterByContainerClasses( array( 'IPS\\gallery\\Album' ), array( 'IPS\\gallery\\Image\\Comment', 'IPS\\gallery\\Image\\Review' ), $albums );
			}
		}
	}
