...
\IPS\Content\MetaData
{
	...
	public static function searchResultExtraData( $items, $member=NULL )
	{
		...
		if ( \count( $imageIds ) )
		{
			...
			$return = array();
			$where	= array( array( 'image_id IN(' . implode( ',', $imageIds ) . ')' ) );

			if( $member )
			{
				$where[] = array( 'image_member_id=?', $member->member_id );
			}
			
			foreach ( \IPS\gallery\Image::getItemsWithPermission( $where, NULL, NULL ) as $image )
			{
