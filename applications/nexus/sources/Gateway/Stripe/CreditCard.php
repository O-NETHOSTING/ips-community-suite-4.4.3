			...
			if ( isset( $sourceData['card']['fingerprint'] ) )
			{
				foreach ( $otherCards as $otherCardId )
				{
					try
					{
						...
						{
							$otherCardData = $this->method->api( "customers/{$profiles[ $this->method->id ]}/cards/{$this->data}", NULL, 'get' );
						}
						
						if ( isset( $otherCardData['fingerprint'] ) and $otherCardData['fingerprint'] === $sourceData['card']['fingerprint'] )
						{
