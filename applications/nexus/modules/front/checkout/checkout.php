...
class _checkout extends \IPS\Dispatcher\Controller
{
	...
	public function _customer()
	{
		...
		if ( $this->needsToLogin and !\in_array( \IPS\Login::registrationType(), array( 'disabled', 'redirect' ) ) )
		{
			...
			$form->add( new \IPS\Helpers\Form\Email( 'email_address', $guestData ? $guestData['member']['email'] : ( $postBeforeRegister ? $postBeforeRegister['email'] : NULL ), TRUE, array( 'accountEmail' => TRUE, 'maxLength' => 150 ) ) );
			$form->add( new \IPS\Helpers\Form\Password( 'password', NULL, TRUE, array( 'protect' => TRUE, 'showMeter' => \IPS\Settings::i()->password_strength_meter, 'checkStrength' => TRUE ) ) );
		...
		if ( $values = $form->values() )
		{
			...
			if ( $this->needsToLogin )
			{
				...
				if( !isset( \IPS\Request::i()->cookie['guestTransactionKey'] ) )
				{
	...
	public function transaction()
	{
		...
		/* If it's a guest make sure we're only able to view for the checkout session */
		if ( !$transaction->member->member_id and ( !isset( \IPS\Request::i()->cookie['guestTransactionKey'] ) or !isset( $transaction->invoice->guest_data['guestTransactionKey'] ) or !\IPS\Login::compareHashes( \IPS\Request::i()->cookie['guestTransactionKey'], $transaction->invoice->guest_data['guestTransactionKey'] ) ) )
		{
			\IPS\Output::i()->error( 'node_error', '2X196/F', 403, '' );
		}
